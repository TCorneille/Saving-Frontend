...
const BalanceCard: React.FC<BalanceCardProps> = ({
  ...
  // ✅ FIXED PATH FOR BALANCE
  const balance = data?.data?.user?.balance ?? 0;

  // Optional user name or email
  const accountName = data?.data?.user?.name ?? "My Account";

  return (
    <div className="w-[1220px] mx-auto px-4">
